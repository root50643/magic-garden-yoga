...
4. 確認大型傳送門中的主要 VRM 是動作回饋中心；桌機右下角及窄螢幕 responsive 鏡像小攝影機都沒有遮住姿勢卡、提示或保持光環。
5. 確認小攝影機同時顯示身體骨架、雙手連線／節點與臉部輪廓；切換高／低畫質時只改 overlay 細節，不改姿勢分數。
6. 逐側緩慢翻掌跨越正負 180° 接縫、張手／握拳、拇指食指輕觸、眨眼與張嘴，確認 VRM 手腕、手指和表情同步且沒有突然翻轉；故意遮住手或臉時，身體姿勢分數仍應正常。
7. 完成至少一個姿勢，確認 Pose Worker 推論、分數與保持計時均正常。
8. 在 Chrome 與 Edge 各驗證一次；再用一台未快取過網站的電腦進行完整五關測試。
...
- [ ] `?avatarDebug=1` 在沒有攝影機時仍可進入闖關頁，檢查主要 VRM、小攝影機版面並帶動左右手腕、手指與表情；不把此結果當成姿勢評分驗收，移除參數後再確認真人 Pose／Hand／Face Landmarker 均可使用。
- [ ] 闖關頁以大型 VRM 為主、小攝影機為輔；雙手／臉部 overlay 清楚可見但不改變瑜珈分數。
- [ ] 左右手正反向越過 ±180° 翻掌接縫都保持連續；靜止手不明顯抖動，張手五指保持伸直，勝利手勢可看出食指／中指間距，握拳或勝利手勢時拇指不穿入手腕／手臂。
- [ ] 第一關 `level-dot--current` 的黃色光暈在桌機可捲動左欄與窄螢幕都沒有被上、左邊界截斷。
